...
$(function() {
    ...
    */
    describe('RSS Feeds', function() {
         ...
         */
        it('has a URL property defined and it is not empty', function() {
         ...
         */
         it('has a name property defined and it is not empty', function(){
            allFeeds.forEach(function(feeds) {
    ...
    describe('The menu', function() {
        ...
        */
        it('display when clicked and does it hides when clicked again', function(){
            var $menuIcon = $('.menu-icon-link');
            //menu display when clicked at first so the menu-hidden tobe(false)
            $menuIcon.trigger('click');
            expect($('body').hasClass('menu-hidden')).toBe(false);
            //menu display when clicked at again so the menu-hidden tobe(true)
            $menuIcon.trigger('click');
            expect($('body').hasClass('menu-hidden')).toBe(true);                  
        });
    ...
    describe('New Feed Selection', function(){
        ...
        beforeEach(function(done){
            loadFeed(1,function(){
                feedUrl = $("a.entry-link").attr("href");
                loadFeed(2, done);
            });
        });

        it('ensures when a new feed is loaded by the loadFeed function that the content actually changes', function(done){
            var currentFeedUrl = $("a.entry-link").attr("href");
            expect(currentFeedUrl).not.toBe(feedUrl);
            done();
        });
    });
}());
